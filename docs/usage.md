...
An example dataset has been uploaded to FigShare for testing ConFindr. You can download it to your current working directory with the following command:
...
### Testing ConFindr using included core-gene databases

**ConFindr has only been validated using rMLST databases. Please use them if possible (`--rmlst`, see instructions below).**

In order to analyze this sample using ConFindr with the pre-included core-gene databases for _Escherichia_, _Listeria_, and _Salmonella_, the parameters you need to provide are:
...
`confindr -i example-data -o output`
...
```bash
  2022-10-17 14:19:46  Welcome to ConFindr 0.8.1! Beginning analysis of your samples...
  2022-10-17 14:19:46  Did not find rMLST databases, if you want to use ConFindr on genera other than Listeria, Salmonella, and Escherichia, you'll need to download them. Instructions are available at https://olc-bioinformatics.github.io/ConFindr/install/#downloading-confindr-databases

  2022-10-17 14:19:46  Beginning analysis of sample example...
  2022-10-17 14:19:46  Checking for cross-species contamination...
  2022-10-17 14:19:58  Extracting conserved core genes...
  2022-10-17 14:20:06  Quality trimming...
  2022-10-17 14:20:07  Detecting contamination...
  2022-10-17 14:20:07  Since this is the first time you are using this database, it needs to be indexed by KMA. This might take a while
  2022-10-17 14:20:42  Done! Number of contaminating SNVs found: 199

  2022-10-17 14:20:42  Contamination detection complete!
```
...
### Testing ConFindr using rMLST databases

The rMLST databases must be downloaded and set up prior to use. You can find the instructions for this [here](https://olc-bioinformatics.github.io/ConFindr/install/#downloading-confindr-databases).

To analyze the test sample using ConFindr with the downloaded rMLST alleles databases, you need to provide an `--input_directory` and an  `--output_name`, as well as the `--rmlst` option on the command line. The command to analyze the test sample in this case would be:

`confindr -i example-data -o output --rmlst`
